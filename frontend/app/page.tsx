...
export default function HomePage() {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-4xl flex-col gap-8 px-6 py-10">
      ...
      <section className="flex flex-col gap-3 rounded-panel border border-hairline bg-panel p-5">
        <h2 className="text-h4 font-semibold text-fg">Upload dataset</h2>
        <div className="flex items-center gap-3">
          <button
            type="button"
            disabled={uploading}
            onClick={() => fileInputRef.current?.click()}
            className="h-7 rounded-control border border-hairline bg-panel-raised px-3 text-xs font-medium text-fg-muted transition-colors hover:bg-hairline disabled:cursor-not-allowed disabled:opacity-50"
          >
            {uploading ? "Uploading…" : "Choose CSV file"}
          </button>
          <span className="text-xs text-fg-subtle">.csv only</span>
        </div>
        <input
          ref={fileInputRef}
          type="file"
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-h4 font-semibold text-fg">Datasets</h2>
        ...
        {!loadError && datasets !== null && datasets.length === 0 && (
          <div className="rounded-panel border border-dashed border-hairline px-5 py-10 text-center text-sm text-fg-subtle">
            ...
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody>
                {datasets.map((ds) => (
                  <tr
                    key={ds.dataset_id}
                    className="h-8 cursor-pointer border-b border-hairline last:border-b-0 hover:bg-panel"
                    onClick={() => router.push(`/datasets/${ds.dataset_id}`)}
                  >
                    <td className="px-3 text-fg">{ds.filename}</td>
                    <td
                    ...
                    <td className="px-3 text-xs text-fg-subtle">
